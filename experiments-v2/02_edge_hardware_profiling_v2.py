"""
===============================================================================
Module Name:       02_edge_hardware_profiling_v2.py
Project:           Chlorophyll-a Soft-Sensor Edge-IoT System
Tier / Subsystem:  Empirical Validation Tier (Benchmark 02 — V2 ONNX)
...
Description:       Performs quantitative edge hardware resource profiling on
                   ...
                   (generated by the V2 ONNX inference engine):
                   ...
                   4. Model artifact storage footprint verification (ONNX binary).

Data Interfaces:
  - Upstream:      experiments-v2/clean_run_data.db (or edge-system/app-v2/sensor_data.db)
                   edge-system/app-v2/model_v2.onnx
  - Downstream:    experiments-v2/edge_hardware_results_v2.json
                   experiments-v2/edge_hardware_summary_tables_v2.md
                   experiments-v2/figures/fig_5_4_latency_distribution.png
                   experiments-v2/figures/fig_5_5_ram_cpu_stability_timeline.png
                   experiments-v2/figures/fig_5_6_energy_tradeoff_comparison.png
...
FALLBACK_DB_PATH = os.path.join(PROJECT_ROOT, "edge-system/app-v2/sensor_data.db")
...
def load_data():
    ...
    if os.path.exists(CLEAN_DB_PATH):
        ...
        try:
            import subprocess
            print("[INFO] [HardwareProfiling] Extracting sensor_data.db from Docker container 'edge_inference_app_v2'...")
            res = subprocess.run(["docker", "cp", "edge_inference_app_v2:/data/sensor_data.db", CLEAN_DB_PATH],
...
def run_profiling():
    print("=" * 65)
    print("  02 — EDGE SYSTEM PERFORMANCE & HARDWARE PROFILING (V2 ONNX)")
    print("=" * 65)
    ...
    # Serialized model size (ONNX binary)
    model_size_bytes = os.path.getsize(MODEL_PATH) if os.path.exists(MODEL_PATH) else 400000
    ...
    # Strategy B: Cloud-Centric Streaming
    t_modem_cloud_tx_sec = samples_per_day * 15.0
    e_modem_cloud_daily_j = (t_modem_cloud_tx_sec * 2.0) + ((86400.0 - t_modem_cloud_tx_sec) * 1.5)
    e_pi_cloud_daily_j = 86400.0 * 2.7
    ...
    all_results = {
        "metadata": {
            ...
            "runtime_environment": "Docker Engine + Eclipse Mosquitto 2.0 + Python 3.11 + ONNX Runtime",
            "model_format": "ONNX (V2)",
    ...
    json_path = os.path.join(SCRIPT_DIR, "edge_hardware_results_v2.json")
    ...
    ax1.set_title("Edge Inference Latency Distribution (ONNX V2)", fontsize=12, fontweight="bold")
    ...
    time_hours = sample_indices * 15 / 60
    ...
    ax1.set_title(f"RAM Stability Analysis Over Continuous Stream ({len(ram_vals):,} Samples / {time_hours[-1]:.0f} Simulated Hours) — ONNX V2",
    ...
    ax2.set_title("Edge Node CPU Overhead Profile (ONNX V2)", fontsize=12, fontweight="bold")
    ...
    width = 0.35
    ax.bar(x - width/2, edge_vals, width, label="Edge Inference — ONNX V2 (Smart 24h Batching)", color="#0a3d62")
    ...
    ax.set_title("Energy Consumption Trade-Off: Edge ONNX V2 vs. Continuous Cloud Streaming", fontsize=12, fontweight="bold")
    ax.set_xticks(x)
    ...
    md_summary = fr"""# Edge System Performance and Hardware Profiling Summary (V2 ONNX)
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **ONNX Runtime Inference** | **{latency_stats['inference_ms']['mean']:.2f} ms** | {latency_stats['inference_ms']['median']:.2f} ms | **{latency_stats['inference_ms']['p95']:.2f} ms** | {latency_stats['inference_ms']['p99']:.2f} ms | {latency_stats['inference_ms']['max']:.2f} ms | — |
...
| **Serialized Model Size** | **{model_size_mb:.2f} MB** | — | — | Lightweight ONNX binary artifact |
...
"""
    md_path = os.path.join(SCRIPT_DIR, "edge_hardware_summary_tables_v2.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write(md_summary)
    print("\n[OK] Edge Hardware Profiling (V2 ONNX) Complete!")
